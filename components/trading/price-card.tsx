...
export function PriceCard({ symbol, price }: PriceCardProps) {
  ...
  return (
    <Card className="border-border bg-card p-4 hover:shadow-lg hover:scale-105 transition-all duration-200 cursor-pointer">
      ...
      {priceChange !== 0 && (
        <div className={`mt-2 h-1 rounded-full ${isPositive ? "bg-green-500" : "bg-red-500"}`} style={{ width: `${Math.min(Math.abs(priceChangePercent), 100)}%` }}></div>
      )}
    </Card>
  )
}